...
typedef struct FolderStack {
    char* folders[MAX_KEYS];
    int top;
} FolderStack;
...
void listar_arquivos_recursiva(const char *caminho_base, BTree* tree) {
        ...
        while ((entrada = readdir(diretorio)) != NULL) {
            if (strcmp(entrada->d_name, ".") != 0 && strcmp(entrada->d_name, "..") != 0) {
                ...
                if (entrada->d_type == DT_REG) {
                    printf("Arquivo %s\n", entrada->d_name);
                    insert(tree, entrada->d_name);
                } else if (entrada->d_type == DT_DIR) {
                    printf("Diretório: %s\n", caminho);

                    // Chame a função para explorar subdiretórios
                    listar_arquivos_recursiva(caminho, tree);
                }
            }
        }
        closedir(diretorio);
